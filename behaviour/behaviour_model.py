...
class BehaviourModel(sc.prettyobj):
    ...
    #     pars = self.load_pars_and_data() # Get a dictionary of parameters
    ...
    def load_pars_and_data(self):
        ...
        pars.expected_age_dist = spdata.get_smoothed_single_year_age_distr(**pars.loc_pars, window_length=self.window_length)
        ...
        pars.age_brackets = spdata.get_census_age_brackets(**pars.loc_pars)
        self.age_brackets = pars.age_brackets
        # mapping
        pars.age_by_brackets = spb.get_age_by_brackets(pars.age_brackets)
        self.age_by_brackets = pars.age_by_brackets

        # Load the contact matrix
        pars.contact_matrices = spdata.get_contact_matrices(pars.datadir, sheet_name=pars.sheet_name)
        ...
        pars.cm_age_brackets = spdata.get_census_age_brackets(**pars.loc_pars, nbrackets=contact_matrix_row)
    ...
    def generate_households(self, pars, 
        ...
        hha_brackets = spdata.get_head_age_brackets(**pars.loc_pars)
        hha_by_size = spdata.get_head_age_by_size_distr(**pars.loc_pars)

        if pars.household_method == 'fixed_ages':

            homes_dic, homes = sphh.generate_all_households_fixed_ages(n_nonltcf, hh_sizes, hha_by_size, hha_brackets, pars.cm_age_brackets, pars.cm_age_by_brackets, pars.contact_matrices, ages_left_to_assign)

        else:
            log.debug("defaulting to 'infer_ages' household generation method. See method notes for description.")
            homes_dic, homes = sphh.generate_all_households_infer_ages(pars.n, n_nonltcf, hh_sizes, hha_by_size, hha_brackets, pars.cm_age_brackets, pars.cm_age_by_brackets, pars.contact_matrices, ltcf_adjusted_age_dist, ages_left_to_assign)
    ...
    def generate_schools(self, pars, n_nonltcf, age_by_uid, homes_by_uids):
        ...
        if pars.with_school_types:
            ...
            school_type_age_ranges = spdata.get_school_type_age_ranges(**pars.loc_pars)
    ...
    def distribute_workers(self, pars, student_age_lists, student_uid_lists, employment_rates, 
        ...
        # Get facility staff
        if pars.with_facilities:
            facilities_staff_uid_lists = spltcf.assign_facility_staff(pars.datadir,
                                                                      pars.location,
                                                                      pars.state_location,
                                                                      pars.country_location,
                                                                      pars.ltcf_staff_age_min,
                                                                      pars.ltcf_staff_age_max,
                                                                      facilities,
                                                                      workers_by_age_to_assign_count,
                                                                      potential_worker_uids_by_age,
                                                                      potential_worker_uids,
        ...
        workplace_size_brackets = spdata.get_workplace_size_brackets(**pars.loc_pars)
        workplace_size_distr_by_brackets = spdata.get_workplace_size_distr_by_brackets(**pars.loc_pars)
...
def make_population(*args, **kwargs):
    ...
    log.debug('Generating a new population...')
    pop = BehaviourModel(*args, **kwargs)
